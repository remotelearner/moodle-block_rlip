...
class rlip_importplugin_version1elis extends rlip_importplugin_base {
    ...
    function __construct($provider = NULL, $manual = false) {
        ...
        foreach ($idfields as $idfield) {
            if (get_config('rlipimport_version1elis','identfield_'.$idfield)) {
        ...
        $tmp = ob_get_contents();
        ob_end_clean();
        error_log("rlipimport_version1elis:__construct(): import_fields_user_update = {$tmp}");
        */
    ...
    protected function get_userid_for_user_actions($record, $filename, &$error, &$errors, &$errsuffix) {
        ...
        if (!get_config('rlipimport_version1elis','allowduplicateemails')) {
        ...
        foreach ($idfields as $idfield) {
            if (isset($record->$idfield)) {
                ...
                if ($usingstdident && get_config('rlipimport_version1elis','identfield_'.$idfield)) {
    ...
    function handle_user_createorupdate($record, $action) {
        ...
        if (!empty($createorupdate)) {
            ...
            $usernameset = get_config('rlipimport_version1elis','identfield_username') && isset($record->username) && $record->username !== '';
            $emailset = get_config('rlipimport_version1elis','identfield_email') && isset($record->email) && $record->email !== '';
            $idnumberset = get_config('rlipimport_version1elis','identfield_idnumber') && isset($record->idnumber) && $record->idnumber !== '';

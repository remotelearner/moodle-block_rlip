...
class rlip_importplugin_version1 extends rlip_importplugin_base {
    ...
    function __construct($provider = NULL, $manual = false) {
        ...
        foreach ($idfields as $idfield) {
            if (get_config('rlipimport_version1','identfield_'.$idfield)) {
    ...
    function handle_user_createorupdate($record, $action) {
        ...
        if (!empty($createorupdate)) {
            ...
            $usernameset = get_config('rlipimport_version1','identfield_username') && isset($record->username) && $record->username !== '';
            $emailset = get_config('rlipimport_version1','identfield_email') && isset($record->email) && $record->email !== '';
            $idnumberset = get_config('rlipimport_version1','identfield_idnumber') && isset($record->idnumber) && $record->idnumber !== '';
    ...
    protected function get_userid_for_user_actions($record, $filename, &$error, &$errors, &$errsuffix) {
        ...
        if (!get_config('rlipimport_version1','allowduplicateemails')) {
        ...
        foreach ($idfields as $idfield) {
            if (isset($record->$idfield)) {
                ...
                if ($usingstdident && get_config('rlipimport_version1','identfield_'.$idfield)) {
